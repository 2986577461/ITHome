<template>
  <div class="modal-overlay" v-if="governStore.addStudentVisible" @click.self="governStore.addStudentVisible = false">
    <div class="modal-card wide">
      <button class="modal-close" @click="governStore.addStudentVisible = false">
      ...
      <div class="dialog-head">
        <div class="dialog-head-icon">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 11a4 4 0 100-8 4 4 0 000 8zM23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>
        </div>
        <h2>新学员申请</h2>
        <p v-if="governStore.newcomers.length">{{ governStore.newcomers.length }} 人待审核</p>
      </div>

      <div v-if="governStore.newcomers.length === 0" class="empty-state">
        <svg width="48" height="48" viewBox="0 0 48 48" fill="none"><path d="M34 42v-4a8 8 0 00-8-8H10a8 8 0 00-8 8v4M18 22a8 8 0 100-16 8 8 0 000 16z" stroke="currentColor" stroke-width="1.5" opacity=".3" stroke-linecap="round" stroke-linejoin="round"/></svg>
        <p>暂无申请</p>
      </div>

      <div v-else class="student-list">
        <div v-for="(s, i) in governStore.newcomers" :key="s.id || i" class="student-card">
          <div class="sc-body">
            <div class="sc-header">
          ...
          <div class="sc-actions">
            <button class="sc-btn-approve" @click="handleApprove(s.id)">
              <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M4 8l3 3 5-6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
              通过
            </button>
...
async function handleApprove(id) {
  ...
  if (resp) {
    ...
    governStore.newcomers = governStore.newcomers.filter((item) => item.id !== id);
  } else {
...
async function handleRefuse(id) {
  ...
  if (resp) {
    ...
    ElMessage.error("操作失败");
    if (s) governStore.newcomers.push(s);
  }
}
</script>
...
.sc-name { font-size: 18px; font-weight: 700; }
.sc-id { font-size: 13px; color: var(--color-text-tertiary); }
...
.sc-label {
  color: var(--color-text-tertiary); min-width: 36px; flex-shrink: 0;
}
.sc-intro {
