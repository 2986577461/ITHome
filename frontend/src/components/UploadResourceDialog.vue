<template>
  <div class="modal-overlay" v-if="uploadStore.loadFileVisible" @click.self="uploadStore.loadFileVisible = false">
      ...
      <form @submit.prevent="submitUpload" class="upload-form">
        <div class="form-row">
          <div class="form-field">
            <label>标题</label>
            <input v-model="resource.head" type="text" required placeholder="请输入标题" />
          </div>
        </div>
          ...
          <div class="upload-box">
            ...
            <div class="upload-zone" @click="coverInput.click()">
              ...
              <div v-else class="upload-placeholder">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none"><path d="M12 4v16M4 12h16" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>
                <span>添加封面</span>
              </div>
            </div>
            <input ref="coverInput" type="file" accept=".jpg,.jpeg,.png" @change="onCoverChange" hidden />
          </div>
          <div class="upload-box">
            ...
            <div class="upload-zone" @click="fileInput.click()">
              ...
              <div v-else class="upload-placeholder">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/><path d="M14 2v6h6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
                <span>选择文件</span>
              </div>
            </div>
        ...
        <div class="form-field">
          <label>简介</label>
          <textarea v-model="resource.introduce" class="textarea" required placeholder="请输入资源简介"></textarea>
        </div>

        <button type="submit" class="btn-primary submit-btn" :disabled="uploading">
          {{ uploading ? "上传中..." : "提交" }}
        </button>
      </form>
    </div>
...
.upload-box { flex: 1; display: flex; flex-direction: column; gap: 6px; }
.upload-label { font-size: 13px; font-weight: 600; color: var(--color-text); }
.upload-zone {
  height: 160px;
...
.upload-zone:hover { border-color: var(--color-accent); background: rgba(0,113,227,0.03); }
.upload-placeholder {
  display: flex; flex-direction: column; align-items: center;
  gap: 8px; color: var(--color-text-tertiary); font-size: 13px;
}
.upload-preview { width: 100%; height: 100%; object-fit: cover; }
.file-name { font-size: 13px; color: var(--color-text); padding: 8px; }
.submit-btn { align-self: center; padding: 12px 48px; margin-top: 8px; }
