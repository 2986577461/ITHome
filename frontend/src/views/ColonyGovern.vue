          ...
          <div class="gov-bar">
            <div class="gov-actions">
              <button class="gov-btn-approve" @click="governStore.toggleAdd()">
                ...
                审核新学员
                <span
                  v-if="governStore.newcomers.length"
                  class="gov-badge-dot"
                  >{{ governStore.newcomers.length }}</span
                >
              </button>
              <button
                class="btn-secondary"
...
async function doRemove() {
  var studentIds = selected.value.map(function (i) {
    return governStore.member[i].studentId;
  });
  await removeBatch(studentIds);
...
onMounted(async function () {
  try {
    ...
    await getAllNewcomers().then(function (r) {
      if (r && r.data) governStore.newcomers = r.data;
    });
  } catch (e) {}
