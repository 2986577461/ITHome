<template>
  <div class="resource-page">
          ...
          <div v-for="r in pagedResources" :key="r.id" class="res-card">
            <div class="res-cover" @click="download(r.objectName)">
              ...
              <div class="res-cover-overlay">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                  <path
                    d="M12 3v12M7 10l5 5 5-5M5 17h14v3H5z"
                    stroke="currentColor"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
            ...
            <div class="res-body">
              <div class="res-title-row">
                <h3>{{ r.head }}</h3>
                <button
                  v-if="r.studentId === userStore.studentId"
                  class="res-del"
    ...
    <button
      ...
      class="fab"
      @click="uploadStore.loadFileVisible = true"
    >
...
import {
  ...
  getDownloadUrl,
  deleteById,
} from "@/request/axiosForResources.js";
...
var totalPages = computed(function () {
  return Math.max(1, Math.ceil(resources.value.length / pageSize));
});
...
async function doDelete() {
  ...
  resources.value = resources.value.filter(function (x) {
    return x.id !== toDelete.value.id;
  });
...
.res-cover:hover .res-cover-overlay {
  opacity: 1;
}
.res-body {
  padding: 24px;
